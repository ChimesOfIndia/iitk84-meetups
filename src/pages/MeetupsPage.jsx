...
const FILTER_GROUPS = [
  ...
  { label: 'Chicago & Midwest', value: 'chicago' },
  { label: 'New York & Tri-State', value: 'new_york' },
...
export default function MeetupsPage() {
  ...
  const load = async () => {
    ...
    if (data && data.length > 0) {
      const { data: rsvps } = await supabase.from('rsvps').select('meetup_id, status, with_spouse, extra_guests').in('meetup_id', data.map(m => m.id))
      const counts = {}
      ;(rsvps || []).forEach(r => {
  ...
  const cardTitle = (m) => {
    ...
    if (parts.length === 0) return m.meetup_type === 'visit' ? `${m.visitor_names} visiting` : `${cityLabel(m)} Meetup`
    return parts.join(' — ')
  }
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sortBy === 'city') return cityLabel(a).localeCompare(cityLabel(b))
    return 0
  })
    ...
    <div>
      <div className="top-bar">
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button className={`filter-chip ${sortBy === 'city' ? 'active' : ''}`} onClick={() => setSortBy('city')} style={{ fontSize: 11 }}>📍 City</button>
        </div>
      </div>
      <div className="page">
